...
class TableRenderer:
            ...
            for x in range(len(table[y])):  # foreach block in row, but with indexes.
                ...
                if block != "":
                    if x % 2 == 0:  # if x is even, then it must be monday wednesday or friday
                        ...
                        if x == 4:
                            if table[y][x - 1] == "" and table[y + 1][x - 1] == "" and table[y - 1][x - 1] == "":
                                for i in range(5):
                                    pretty_table[y + 1][x - i] = self.create_special_bottom_cell(chunk_width)
                        ...
                        if x == 3:  # A check to see if adjacent blocks meet requirements for the full line across
                            if table[y + 2][x + 1] == "":
                                for i in range(5):
                                    pretty_table[y + 2][x - i] = self.create_special_bottom_cell(chunk_width)
                    if y > 0:
                        pretty_table[y - 1][x] = self.create_bottom_cell(chunk_width)
                else:
                    if (pretty_table[y][x]) == "":
    ...
    def print_pretty_table(self):
        ...
        while line < len(self.pretty_table) * 3:
            ...
            if line % 3 == 0:  # There are always 3 newlines in every "cell", print the time next to the start of every cell
                print(self.index_to_time(time_index).ljust(5), end="")
                time_index += 1
            else:
                print("".ljust(5), end="")
    ...
    def index_to_time(self, index_int):
